...
DataCopierBuilder& DataCopierBuilder::AddDirectPath(MemoryType src_type, MemoryType dest_type, CopyFunction func) {
...
DataCopier::DataCopier(std::map<std::pair<MemoryType, MemoryType>, CopyFunction> copy_matrix)
    : copy_matrix_(std::move(copy_matrix)) {}

CopyFunction DataCopier::FindCopier(MemoryType src_type, MemoryType dest_type) const {
    auto it = copy_matrix_.find({src_type, dest_type});
    return (it != copy_matrix_.end()) ? it->second : nullptr;
}

bool DataCopier::Copy(const DataSource& src, MemoryType dest_type, void* dest_ptr) const {
    ...
    if (src.type != MemoryType::DRAM && dest_type != MemoryType::DRAM) {
        VLOG(1) << "No direct copier. Attempting fallback via DRAM for "
                << MemoryTypeToString(src.type) << " -> " << MemoryTypeToString(dest_type);
        ...
        if (to_dram_copier && from_dram_copier) {
            ...
            if (!temp_dram_buffer) {
                LOG(ERROR) << "Failed to allocate temporary DRAM buffer for fallback copy.";
                return false;
            }
            ...
            DataSource temp_dram_source = {temp_dram_buffer.get(), src.size, MemoryType::DRAM};
            if (!from_dram_copier(temp_dram_source, dest_ptr)) {
                LOG(ERROR) << "Fallback copy failed at Step B (DRAM -> Destination)";
                return false;
            }
            return true;
        }
    ...
    LOG(ERROR) << "No copier registered for transfer from memory type "
               ...
               << MemoryTypeToString(dest_type) << ", and fallback path is not available.";
    return false;
}
